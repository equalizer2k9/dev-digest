...
| [`findings-column.md`](findings-column.md) | implemented | FINDINGS column between SCORE and STATUS on the PR list: one `SeverityBadge compact` chip per non-zero severity from `PrMeta.findings_counts`, deep-linking to `?tab=findings`; hovering the cell opens a read-only, `position: fixed` preview of each agent's latest findings, fetched lazily from `GET /pulls/:id/reviews`. |
...
| [`skills-lab.md`](skills-lab.md) | draft | Client half of Skills Lab: SKILLS LAB sidebar section, `/skills` card grid with a side-panel preview, create + file/zip import modals, `/skills/:id` (Config · Preview · Versioning with Diff/Restore), agent tiles as a grid, an agent `Skills` tab with search and drag-and-drop over enabled skills, a shared `ConfirmDialog`, and per-skill token blocks in the Run Trace. |
